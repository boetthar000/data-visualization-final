...
const svg = d3.select("#beeswarm-svg-2")
  .append("svg")
...
function initializeControls() {
    ...
    d3.select("#play-pause-button-2").on("click", togglePlayPause);
}
...
async function runSimulation() {
    ...
    while(isPlaying && virtualYear <= 2013) {
        ...
        d3.select("#year-input-2").property("value", selectedYear);
        updateChart();
...
async function togglePlayPause() {
    isPlaying = !isPlaying;
    d3.select("#play-pause-button-2").text(isPlaying ? "Pause" : "Play");
